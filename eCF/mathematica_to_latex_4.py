...
file=open('Identities.txt','r').read()
...
#listOfFunctions=["Sqrt", "Polygamma", 
...
def replaceGreekVars(s):
    ...
    for gvar in listOfGVars:
        ...
        #replacing * with space for multiplication, easier to understand
...
for index in range(len(file)):
    if file[index: index+2]=="(*":
        ...
        position=file.find("\n", index)
        position=file.find("\n", position+1)
        newFile.write(equationSetUp(file[index: position]))
newFile.close()
