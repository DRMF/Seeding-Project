    ...
    def test_reference_placer(self):
        self.assertEquals(reference_placer(['\\section{Racah}\\index{Racah polynomials}',
                  ...
                  '\\label{DefRacah}', '& &R_n(\lambda(x);\alpha,\beta,\gamma,\delta)\nonumber\\'
                    ...
                    , '\\label{DefRacah}', '& &R_n(\lambda(x);\alpha,\beta,\gamma,\delta)\nonumber\\'
                    , '& &{}=\hyp{4}{3}{-n,n+\alpha+\beta+1,-x,', '\end{eqnarray}'
                    , 'with $N$ a nonnegative integer.%Begin KLSadd additions\\subsection*{9.1 Racah}\\label{sec9.1}\\paragraph{\\large\\bf KLSadd: Hypergeometric representation}In addition to (9.1.1) we have%End of KLSadd additions', '\\subsection*{References}',
        ...
        self.assertEquals(reference_placer(['\\section{Racah}\\index{Racah polynomials}',
                ...
                , '\\label{DefRacah}', '& &R_n(\lambda(x);\alpha,\beta,\gamma,\delta)\nonumber\\'
                , '& &{}=\hyp{4}{3}{-n,n+\alpha+\beta+1,-x,', '\end{eqnarray}'
                , 'with $N$ a nonnegative integer.%Begin KLSadd additions\\subsection*{14.1 Racah}\\label{sec9.1}\\paragraph{\\large\\bf KLSadd: Hypergeometric representation}In addition to (9.1.1) we have%End of KLSadd additions', '\\subsection*{References}',
                '\cite{Askey89I}, \cite{AskeyWilson79}, \cite{AskeyWilson85}'])
